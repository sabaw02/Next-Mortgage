...
return(
    <div className="min-[550px]:hidden relative mt-4">
        <div className="w-25  h-20 content-center   ">
            {/* دکمه */}
            <button
                onClick={()=> setIsOpen (!isOpen)} 
                className="flex flex-col gap-2 w-20 h-15 mx-auto items-center justify-center ">
                <span className={`content-center block w-7 h-0.5 bg-black transition-all duration-300 
            ...
            {/* منو کشویی */}
            <div className={`rounded-br-3xl rounded-tr-3xl z-10 sticky top-36 right-20 h-auto w-40 bg-white dark:bg-[#334155] border-r-2 border-r-[#1c5387] shadow-lg 
             transition-transform duration-300 
                    ${isOpen ? 'translate-x-0' : '-translate-x-500'}
                `}>
                <div className="flex flex-col h-auto gap-1 p-3 ">
                    {headerNavItemes.map((item)=>{
